...
const effects = ['Volume', 'Panoramic', 'Sustain', 'Reverb', 'Chorus', 'ADSR', 'Echo', 'Distortion'];
...
effectShortcutBtn1.textContent = 'Panoramic';
effectShortcutBtn2.textContent = 'Echo';
effectShortcutBtn3.textContent = 'Reverb';
effectShortcutBtn4.textContent = 'Volume';
...
effectShortcutBtn1.addEventListener('click', () => { currentPage = 1; OpenEffectsWindow(currentPage); });
...
effectShortcutBtn4.addEventListener('click', () => { currentPage = 0; OpenEffectsWindow(currentPage); });
...
panoramicKnob.addEventListener('mousedown', () => {
    if (effectsContainer.style.display != 'none' && currentPage == 1) {
        panoramicDragging = true;
    }
});
panoramicView.addEventListener('mousedown', (e) => {
    if (effectsContainer.style.display != 'none' && currentPage == 1) {
...
function DisplayReverbSlider2(reverbSVG, value, reverbBox) {
    ...
    handle.addEventListener('mousedown', () => {
        if (effectsContainer.style.display != 'none' && currentPage == 3) {
            reverbDragging = { state: true, freq: reverbSVG.id == 'reverbFreqSVG' };
...
function DisplayChorusSlider2(chorusSVG, chorusBox, slider) {
    ...
    handle.addEventListener('mousedown', () => {
        if (effectsContainer.style.display != 'none' && currentPage == 4) {
            chorusDragging = { state: true, slider: slider };
...
function CreateADSRKnob(index, adsrSVG, dx, dy) {
    ...
    knob.addEventListener('mousedown', () => {
        if (effectsContainer.style.display != 'none' && currentPage == 5) {
            adsrDragging = { state: true, setting: index };
...
function DisplayEchoSliders2(echoSVG, echoBox, slider) {
    ...
    handle.addEventListener('mousedown', () => {
        if (effectsContainer.style.display != 'none' && currentPage == 6) {
            echoDragging = { state: true, slider: slider };
...
distortionResetBtn.addEventListener('click', () => {
    ...
    const interval = setInterval(() => {
        ...
        distortionMixSlider.value = distortionMix;
        distortionBandWidthSlider.value = distortionBandWidth;
        distortionLowPassSlider.value = distortionLowPass;
...
const distortionMixSlider = CreateInput('distortionMixSlider', '0', '1', distortionMix);
{
    distortionMixSlider.step = '0.01';
    distortionMixSlider.value = distortionMix;
    distortionMixSliderBox.appendChild(distortionMixSlider);
}
const distortionBandWidthSlider = CreateInput('distortionBandWidthSlider', '0', '1', distortionBandWidth);
{
    distortionBandWidthSlider.step = '0.01';
    distortionBandWidthSlider.value = distortionBandWidth;
    distortionBandWidthSliderBox.appendChild(distortionBandWidthSlider);
}
const distortionLowPassSlider = CreateInput('distortionLowPassSlider', '0', '1', distortionLowPass);
{
    distortionLowPassSlider.step = '0.01';
    distortionLowPassSlider.value = distortionLowPass;
    distortionLowPassSliderBox.appendChild(distortionLowPassSlider);
}

distortionMixSlider.addEventListener('input', () => {
    distortionMix = parseFloat(distortionMixSlider.value);
...
distortionBandWidthSlider.addEventListener('input', () => {
    distortionBandWidth = parseFloat(distortionBandWidthSlider.value);
...
distortionLowPassSlider.addEventListener('input', () => {
    distortionLowPass = parseFloat(distortionLowPassSlider.value);
...
function UpdatePosition() {
    if (currentPage == 0) {
        ...
        //Nothing to do here for distortion but I keep this if statement "in case of" .. ??
    }
